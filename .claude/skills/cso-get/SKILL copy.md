...
# [要求]：
**[GP-DRIVEN-DECONSTRUCTION]**：收到指令后，必须首先通过网络检索一些信息，对 GP 的原始问题：进行物理拆解。你必须识别出问题背后的标的物、风险维度、以及隐含的逻辑断点。
...
# 二、标准回复，严禁使用任何 Markdown 格式（如代码块、粗体等），直接输出以下纯文本
```
...
[D-P-时效-定级-序号] | [Target Data:ID] | | Fact_Summary:消息摘要 | [Ref:锚点A,锚点x,]
[信源指纹索引库 (Source Fingerprint Index)]
-锚点A | 消息来源 | 物理路径/URL [执行MD5/短链压缩]
-锚点B | 消息来源 | 物理路径/URL [执行MD5/短链压缩]
...
[输出主要是简体中文]
[根据参数 {{ recon_level }}严格锁死边界]
->插入 `references/constraints/GB-01.md`
[数据编码与置信度评级] [DD]
->插入 `references/syslawer/DD.md`
